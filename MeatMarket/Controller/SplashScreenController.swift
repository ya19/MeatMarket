    ...
    @objc func checkRecipesCount(_ timer:Timer){
        ...
        if allRecipesSize == serverRecipesCount{
            ...
            for meatCutId in self.currentAllMeatCuts.keys{
                ...
                storageRef.child("\(meatCut!.name).png").downloadURL { (url, error) in
                    if let error = error {
        ...
        meatCutsRef.observeSingleEvent(of: .value, with: { (meatCutsData) in
            ...
            for meatCutID in meatCuts.keys{
                ...
                recipeRef.child(meatCutID).observeSingleEvent(of: .value) { (recipesData) in
                    ...
                    let recipesIdData = recipesData.value as! [String:Any]
                    print(recipesIdData.keys.count,"expected count")
                    self.serverRecipesCount += recipesIdData.keys.count
                    for recipeId in recipesIdData.keys{
            ...
            Timer.scheduledTimer(timeInterval: 0.25, target: self, selector: #selector(self.checkRecipesCount(_:)), userInfo: nil, repeats: true)
